...
- `kage dispatch --name <label> --prompt <instructions>` — From a connector run, start detached one-off agent work and return its run ID immediately. Use this for lengthy ad-hoc connector requests that do not have a task file. The worker uses a verified COMPLETE / CONTINUE / BLOCKED contract and can continue pending remote work in the same run for up to three provider turns.
...
For lengthy ad-hoc work requested through a connector, call `kage dispatch --name <short-label> --prompt <self-contained-instructions>`. It creates a detached child agent run and automatically delivers the final output and artifacts to the source connector. Do not attempt to select an agent or connector: dispatch derives them from the DB-anchored source run, preserves the immutable agent binding, and rejects cross-agent delivery. The child must keep running until all nested commands, sub-agents, polling, and remote operations required by the request are finished; never return an interim progress report as the final result. Dispatch asks the provider for a final COMPLETE / CONTINUE / BLOCKED marker. CONTINUE, a missing marker, a contradicted COMPLETE marker, or abandoned background tasks cause another provider turn in the same run, up to three turns. Once `kage dispatch` returns, tell the user the run ID rather than claiming completion.

Outside a dispatch worker, detached work that will finish after the parent run must call `kage connector send <name> --message <text> [--file "$KAGE_ARTIFACT_DIR/<file>"]` on completion. A dispatch worker must instead remain alive until its requested work is complete. Agent runs may send only through connectors bound to the same DB-anchored agent, and may attach only top-level files from their own `KAGE_ARTIFACT_DIR`; cross-agent sends are rejected.
