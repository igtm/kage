...
- `kage dispatch --name <label> --prompt <instructions>` — From a connector run, start detached one-off agent work and return its run ID immediately. Use this for lengthy ad-hoc connector requests that do not have a task file. The dispatch worker must wait for all nested/background work and return only a completed result or terminal blocker.
