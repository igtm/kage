    ...
    config = GlobalConfig(
        ...
        providers={
            ...
            "codex_json": ProviderConfig(
                command="codex_json", parser="jq", parser_args=".output"
            ),
...
def test_execute_shell_command_with_custom_shell(
    tmp_path: Path, mock_global_config, mocker
):
...
def test_execute_explicit_provider(tmp_path: Path, mock_global_config, mocker):
    ...
    task = TaskDef(
        name="ai_task", cron="* * * * *", prompt="Fix this", provider="codex_json"
    )
    execute_task(tmp_path, task)
...
def test_execute_provider_with_jq(tmp_path: Path, mock_global_config, mocker):
    ...
    task = TaskDef(
        name="jq_task", cron="* * * * *", prompt="Test JQ", provider="jq_provider"
    )
    execute_task(tmp_path, task)
...
def test_execute_inline_jq_parser(tmp_path: Path, mock_global_config, mocker):
    ...
    def side_effect(cmd, **kwargs):
        if cmd[0] == "my_cli":
            return MagicMock(
                returncode=0, stdout='{"output":{"text":"hello"}}', stderr=""
            )
        elif cmd[0] == "jq":
...
def test_execute_claude_normalization(tmp_path: Path, mock_global_config, mocker):
    ...
    task = TaskDef(
        name="claude_task", cron="* * * * *", prompt="Fix this", provider="claude"
    )
